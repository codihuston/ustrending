...
const MapChart = ({ handleClick, realtimeTrendsByState, colorsByTopic }) => {
  ...
  function tooltipStyle(name) {
    ...
    const topicName = realtimeTrendsByState.get(name)
      ? realtimeTrendsByState.get(name)[0].topic
      : null;
    const style = {
      default: {
        fill: "#D6D6DA",
  ...
  function handleMouseEnter(event, name) {
    const dailyTrend = realtimeTrendsByState.get(name);
    if (!dailyTrend) {
