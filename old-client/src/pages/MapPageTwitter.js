...
import MapChartPlaces from "../components/MapChartPlaces";
...
function MapPage(props) {
  ...
  const handleClick = (event, woeid) => {
    if (woeid) {
      const place = places.get(woeid);

      if (place) {
        // TODO: show list of trends for this place
        console.log("TODO: show list of trends for this place", place.name);
      } else {
        console.warn(`No daily trends found for '${woeid}'.`);
      }
    } else {
  ...
  return (
    <Paper>
      <Box p={3}>
        ...
        <MapChartPlaces {...props} handleClick={handleClick} />
