...
const CRON_EXPRESSION_REALTIME_TRENDS =
  process.env.CRON_EXPRESSION_REALTIME_TRENDS ||
  defaults.CRON_EXPRESSION_REALTIME_TRENDS;
...
async function runDailyTrends(cronjobName) {
  ...
  try {
    ...
    const results = processor.process(dailyTrendsByState, comparedGeo);
...
client.on("ready", async function () {
  ...
  const cronRunRealtimeTrends = new CronJob(
    CRON_EXPRESSION_REALTIME_TRENDS,
    async function () {
      const interval = parser.parseExpression(CRON_EXPRESSION_REALTIME_TRENDS);
      console.log(
        `Executing [${CRONJOB_NAME_REALTIME_TRENDS}] cronjob. Next execution scheduled for `,
        interval.next().toISOString()
      );
